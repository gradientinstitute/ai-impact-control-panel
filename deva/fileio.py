...
# def list_algorithms():
#     algorithms = glob(os.path.join(repo_root(), 'algorithms/*/'))
#     metadata_files = {os.path.basename(os.path.normpath(p)): toml.load(
#         os.path.join(p, "metadata.toml")) for p in algorithms}
#     return metadata_files


# TODO: load algorithm
# def load_algorithm(algorithm_name, pfilter=True):
#     # Load all algorithm files
#     algorithm_path = os.path.join(repo_root(), 'algorithms', algorithm_name)
#     print("Scanning ", algorithm_path)
#     input_files = get_all_files(algorithm_path)
#     models = {}
#     for name, fs in input_files.items():
#         fname = fs['metrics']
#         models[name] = toml.load(fname)

#     algorithm = toml.load(os.path.join(algorithm_path, "metadata.toml"))

#     assert len(models) > 0, "There are no candidate models."

#     # Filter efficient set
#     if pfilter:
#         models = remove_non_pareto(models, algorithm)
...
#     if 'primary_metric' in algorithm:
#         primary = algorithm['primary_metric']
#         if primary not in metrics:
#             raise RuntimeError(f'{primary} is not in the algorithm metrics.')

#     algorithm["baseline"] = load_baseline(algorithm_name)
